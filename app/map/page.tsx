...
const FOG = [
  { left: '10%', top: '20%', w: 250 },
  { left: '60%', top: '15%', w: 200 },
  { left: '30%', top: '60%', w: 280 },
  { left: '70%', top: '70%', w: 220 },
]
...
export default function MapPage() {
  ...
  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => {
      ...
      for (const idx of connected) {
        ...
        const fits =
          (dir === 'up' && loc.y < cur.y) ||
          (dir === 'down' && loc.y > cur.y) ||
          (dir === 'left' && loc.x < cur.x) ||
          (dir === 'right' && loc.x > cur.x)
        if (fits) {
  ...
  }, [playerLoc, investigate, moveTo, startAudio])
  ...
  const onTouchEnd = (e: React.TouchEvent) => {
    ...
    for (const idx of connected) {
      ...
      const fits = horizontal
        ? (dx > 0 ? loc.x > cur.x : loc.x < cur.x)
        : (dy > 0 ? loc.y > cur.y : loc.y < cur.y)
      if (fits && (target === null || (horizontal ? Math.abs(loc.x - cur.x) < Math.abs(LOCATIONS[target].x - cur.x) : Math.abs(loc.y - cur.y) < Math.abs(LOCATIONS[target].y - cur.y)))) {
        target = idx
      }
    }
    if (target !== null) { startAudio(); moveTo(target) }
  }
  ...
  return (
    <div onClick={startAudio} onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}
      ...
      {/* Fog */}
      {FOG.map((f, i) => (
        <div key={`fog${i}`} style={{ position: 'absolute', left: f.left, top: f.top, width: f.w, height: f.w * 0.6, borderRadius: '50%', background: 'radial-gradient(ellipse, rgba(139,0,0,0.08), transparent 70%)', filter: 'blur(60px)', pointerEvents: 'none', zIndex: 0 }} />
      ))}
      ...
      {/* TOPBAR */}
      <div style={{ position: 'relative', zIndex: 20, height: 48, background: '#0a0000', borderBottom: '1px solid #2a0000', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 16px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <input type="range" min="0" max="1" step="0.01" value={vol} onChange={e => setVol(+e.target.value)} style={{ width: 60, accentColor: '#8B0000' }} />
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          <span style={{ fontFamily: "var(--font-horror, 'Creepster'), cursive", color: '#cc0000', fontSize: 20, letterSpacing: 4, textShadow: '0 0 10px #ff0000, 0 0 20px #8B0000' }}>MONGLI</span>
          <span style={{ color: '#C4923A', fontSize: 10, fontFamily: 'monospace' }}>{visited.length}/{LOCATIONS.length} {lang === 'es' ? 'investigados' : 'investigated'}</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <button onClick={() => router.push('/')} style={{ background: 'transparent', border: '1px solid #2a0000', color: '#666', fontSize: 10, padding: '3px 8px', fontFamily: 'monospace', cursor: 'pointer' }}>←</button>
          <button onClick={e => { e.stopPropagation(); setLang(l => l === 'es' ? 'en' : 'es'); localStorage.setItem('mongli-lang', lang === 'es' ? 'en' : 'es') }} style={{ background: 'transparent', border: '1px solid #2a0000', color: '#666', fontSize: 10, padding: '3px 8px', fontFamily: 'monospace', cursor: 'pointer' }}>{lang.toUpperCase()}</button>
        </div>
      </div>

      {/* MAP AREA */}
      <div style={{ position: 'relative', width: '100%', height: 'calc(100vh - 48px)', overflow: 'hidden', zIndex: 2 }}>
        <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%, -50%)', width: 420, height: 580 }}>
          ...
          <svg style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', zIndex: 0 }}>
            {CONN_PAIRS.map(([a, b]) => {
              ...
              return (
                <line key={`c${a}-${b}`}
                  x1={la.x + 14} y1={la.y + 14} x2={lb.x + 14} y2={lb.y + 14}
                  stroke={done ? '#8B0000' : '#3a0010'} strokeWidth={done ? 2 : 1}
                  strokeDasharray={done ? '' : '6,4'} opacity={done ? 0.8 : 0.4}
                />
              )
            })}
          </svg>
          ...
          {LOCATIONS.map((l) => {
            ...
            return (
              <div key={l.id} style={{
                position: 'absolute', left: l.x - 14, top: l.y - 14,
                width: 60, display: 'flex', flexDirection: 'column', alignItems: 'center', zIndex: 3,
              }}>
                <div style={{
                ...
                <span style={{
                  fontSize: 11, fontWeight: 'bold', marginTop: 3,
                  color: isActive ? '#ff6644' : isVisited ? '#C4923A' : '#e8d5b0',
                  textShadow: isActive ? '0 0 6px #ff0000' : 'none',
      ...
      {/* Mobile joystick */}
      <div className="mobile-joy" style={{ position: 'fixed', bottom: 24, left: 24, zIndex: 30 }}>
        <div style={{ width: 100, height: 100, borderRadius: '50%', background: 'rgba(139,0,0,0.15)', border: '1px solid rgba(139,0,0,0.3)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 2, backdropFilter: 'blur(4px)' }}>
          <button onClick={() => { startAudio(); const c = CONNECTIONS[playerLoc]; const cur = LOCATIONS[playerLoc]; const t = c.find(i => LOCATIONS[i].y < cur.y); if (t !== undefined) moveTo(t) }} style={{ background: 'none', border: 'none', color: '#cc0000', fontSize: 20, lineHeight: 1, padding: 4 }}>▲</button>
          <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
            <button onClick={() => { startAudio(); const c = CONNECTIONS[playerLoc]; const cur = LOCATIONS[playerLoc]; const t = c.find(i => LOCATIONS[i].x < cur.x); if (t !== undefined) moveTo(t) }} style={{ background: 'none', border: 'none', color: '#cc0000', fontSize: 20, lineHeight: 1, padding: 4 }}>◄</button>
            <button onClick={() => { startAudio(); investigate() }} style={{ background: 'none', border: 'none', color: '#C4923A', fontSize: 11, fontFamily: 'monospace', fontWeight: 'bold', padding: 4 }}>E</button>
            <button onClick={() => { startAudio(); const c = CONNECTIONS[playerLoc]; const cur = LOCATIONS[playerLoc]; const t = c.find(i => LOCATIONS[i].x > cur.x); if (t !== undefined) moveTo(t) }} style={{ background: 'none', border: 'none', color: '#cc0000', fontSize: 20, lineHeight: 1, padding: 4 }}>►</button>
          </div>
          <button onClick={() => { startAudio(); const c = CONNECTIONS[playerLoc]; const cur = LOCATIONS[playerLoc]; const t = c.find(i => LOCATIONS[i].y > cur.y); if (t !== undefined) moveTo(t) }} style={{ background: 'none', border: 'none', color: '#cc0000', fontSize: 20, lineHeight: 1, padding: 4 }}>▼</button>
        </div>
